...
function validateCheck(value: unknown): VerificationResult {
  ...
  if (
    ...
    !isVerificationDiagnosticReport(check.diagnostics)
  )
    throw new ReviewTransitionError(
      "invalid-check",
      "check.diagnostics must be a valid verification diagnostic report.",
    );
  return {
    name,
    ...
    ...(check.diagnostics === undefined
      ? {}
      : { diagnostics: check.diagnostics as VerificationDiagnosticReport }),
  };
}
