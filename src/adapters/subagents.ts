...
export class SubagentsRpcClient {
  ...
  async #spawn(
    ...
    const task = [
      ...
      "Reviewer remediation is pre-authorized only for findings classified as patch-defect that stay inside the accepted builder contract and do not change product/scope/UX/protected-branch/security authority. Apply those in-contract fixes without asking the supervisor, commit through forge_commit kind review-fixes, rerun applicable verification, update the prepared PR head, and run a fresh complete reviewer panel up to maxReviewRounds. A contract-gap finding returns to investigation for a superseding FORGE:CONTRACT before any new edit; it is not a quality-based GATED outcome. Escalate only genuinely out-of-contract or product/authority decisions.",
    ...
    const boundedTask = input.node
      ? [
          ...
          "For every non-review node, return artifact as a forgedock.phase-artifact/v1 object whose phase matches this node. Supply typed facts only; never author Markdown or markers. Investigation must include actual taskType, complexity, evidence, decomposition, skipped phases, and acceptance checks. Plan must publish the investigation's concise builder brief: relevant live path and callers/consumers, invariants and applicable transitions, historical constraints, ordered implementation route, acceptance mapping, explicit limits, and unverified behavior, alongside allowed paths and forbidden changes. Implementation must include the real commit SHA and changed-file statistics. Verification must name every check and use passed, failed, skipped, pending, unknown, not-configured, or policy-exempt truthfully. For prepare-pr, call forge_prepare_review and return the exact PR/head/domains. The trusted parent validates the object and deterministically renders GitHub Markdown. Review nodes return only the typed reviewer result.",
