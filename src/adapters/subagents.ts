...
export class SubagentsRpcClient {
  ...
  async spawnWorkOn(input: WorkOnLaunchInput): Promise<SubagentSpawnReceipt> {
    ...
    const task = [
      ...
      `At review, obtain the committed patch through forge_diff and launch ${FORGE_REVIEW_CORRECTNESS_AGENT} and ${FORGE_REVIEW_SECURITY_AGENT} together with runs.all in one workflowScript, context fresh, cwd set to the assigned worktree, and outputSchema set to this exact schema: ${reviewerSchema}. The required policy reviewer roster is: ${input.policy.review.required.join(", ")}. Return a structured result for every configured required reviewer and do not silently substitute names.`,
