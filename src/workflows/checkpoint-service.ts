import type { GitHubIssueProjector } from "../adapters/github-projection.ts";
import {
  createRunEvent,
  type RunEvent,
  type RunEventType,
...
export interface CheckpointParameters {
  phase: RunPhase;
  attempt: number;
...
export class CheckpointProjectionService {
  async project(input: ProjectCheckpointInput): Promise<void> {
    if (input.params.action === "queue") return;
    if (input.params.action !== "start") {
    ...
    const workflowLabel = workflowLabelForCheckpoint(input.params);
    if (workflowLabel) {
...
export function checkpointStartEvent(
  binding: CheckpointBinding,
  params: CheckpointParameters,
  repository: string,
  sequence: number,
  ...
  return createRunEvent({
    ...
    actor: { kind: "extension", sessionId, leaseEpoch },
    idempotencyKey: `phase:${params.phase}:${params.attempt}:${params.action}`,
