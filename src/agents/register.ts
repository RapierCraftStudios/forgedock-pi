...
At plan completion, provide the typed builder contract object to forge_checkpoint in addition to the canonical Markdown artifact. The contract must contain schema forgedock.builder-contract/v1, revision 1, the frozen base SHA, and normalized exact paths or directory rules ending in /**. Do not include .pi or .git paths. Every later prepare-worktree, implementation, verification, and review checkpoint is bound to the accepted contract hash. At review, keep fixes within the accepted paths; an out-of-contract fix requires forge_commit kind review-fixes with an explicit revisioned contract extension carrying the prior hash, added paths, rationale, and finding IDs.
