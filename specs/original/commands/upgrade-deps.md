...
UPGRADE_TITLE="feat(deps): upgrade {PACKAGE} from {CURRENT} to {LATEST} ({SEMVER_LEVEL})"
SCRATCHPAD="${FORGE_SCRATCHPAD:-$PWD/.forge-scratch}"
AGENT_TOKEN="${AGENT_ID:-${HOSTNAME:-upgrade-deps}-$$}"
mkdir -p "$SCRATCHPAD"
UPGRADE_BODY_MARKER="FORGE:BODY-INTEGRITY:upgrade-deps_{PACKAGE}_${AGENT_TOKEN}"
UPGRADE_BODY_FILE=$(mktemp "$SCRATCHPAD/upgrade-deps_${AGENT_TOKEN}.XXXXXX.md")
cat > "$UPGRADE_BODY_FILE" <<'ISSUE_EOF'
## Problem
...
The repository manifest currently resolves `{PACKAGE}` at `{CURRENT}` while `{LATEST}` is available. Compatibility impact remains an investigation question.
...
ISSUE_EOF
printf '\n<!-- %s -->\n' "$UPGRADE_BODY_MARKER" >> "$UPGRADE_BODY_FILE"
ISSUE_SKILL_OUTPUT=$(Skill(skill="issue", args="--title \"$UPGRADE_TITLE\" --body-file \"$UPGRADE_BODY_FILE\" --label enhancement"))
rm -f "$UPGRADE_BODY_FILE"
UPGRADE_ISSUE=$(printf '%s\n' "$ISSUE_SKILL_OUTPUT" | sed -n \
  -e 's/.*ISSUE_CREATE_RESULT:CREATED number=\([0-9][0-9]*\).*/\1/p' \
  -e 's/.*ISSUE_CREATE_RESULT:DEDUP number=\([0-9][0-9]*\).*/\1/p' | head -1)
[ -n "$UPGRADE_ISSUE" ] || { echo "ERROR: dependency issue hook returned no verified issue number" >&2; exit 1; }
CREATED_ISSUES+=("$UPGRADE_ISSUE")
fi
```
...
BATCH_TITLE="feat(deps): batch patch upgrades — {COUNT} packages ({DATE})"
SCRATCHPAD="${FORGE_SCRATCHPAD:-$PWD/.forge-scratch}"
AGENT_TOKEN="${AGENT_ID:-${HOSTNAME:-upgrade-deps}-$$}"
mkdir -p "$SCRATCHPAD"
BATCH_BODY_MARKER="FORGE:BODY-INTEGRITY:upgrade-deps_batch_${AGENT_TOKEN}"
BATCH_BODY_FILE=$(mktemp "$SCRATCHPAD/upgrade-deps-batch_${AGENT_TOKEN}.XXXXXX.md")
cat > "$BATCH_BODY_FILE" <<'BATCH_ISSUE_EOF'
## Problem
...
The repository lock and manifest resolve the listed packages below their currently available patch releases. Individual compatibility remains subject to investigation.
...
Candidate investigation starting points (not mutation authority):
1. `package.json` / `requirements.txt` / `Cargo.toml` — inspect current constraints
2. Lockfile — inspect current resolved versions
...
ISSUE_SKILL_OUTPUT=$(Skill(skill="issue", args="--title \"$BATCH_TITLE\" --body-file \"$BATCH_BODY_FILE\" --label enhancement"))
rm -f "$BATCH_BODY_FILE"
BATCH_ISSUE=$(printf '%s\n' "$ISSUE_SKILL_OUTPUT" | sed -n \
  -e 's/.*ISSUE_CREATE_RESULT:CREATED number=\([0-9][0-9]*\).*/\1/p' \
  -e 's/.*ISSUE_CREATE_RESULT:DEDUP number=\([0-9][0-9]*\).*/\1/p' | head -1)
[ -n "$BATCH_ISSUE" ] || { echo "ERROR: batch dependency issue hook returned no verified issue number" >&2; exit 1; }
CREATED_ISSUES+=("$BATCH_ISSUE")
fi
```
