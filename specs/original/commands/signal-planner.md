...
```bash
TRACKER_TITLE="investigate: plan response to {SIGNAL.name}"
SCRATCHPAD="${FORGE_SCRATCHPAD:-$PWD/.forge-scratch}"
AGENT_TOKEN="${AGENT_ID:-${HOSTNAME:-signal-planner}-$$}"
mkdir -p "$SCRATCHPAD"
TRACKER_BODY_MARKER="FORGE:BODY-INTEGRITY:signal-planner_{SIGNAL.name}_${AGENT_TOKEN}"
TRACKER_BODY_TMPFILE=$(mktemp "$SCRATCHPAD/signal-planner_${AGENT_TOKEN}.XXXXXX.md")
trap 'rm -f "$TRACKER_BODY_TMPFILE"' EXIT
cat > "$TRACKER_BODY_TMPFILE" <<'TRACKER_EOF'
## Problem

Signal `{SIGNAL.name}` requires a dependency-ordered investigation and work plan.

## Root Cause

Root cause unknown — investigation required. Current signal evidence is preserved below.

## Affected Files

Candidate investigation starting points (not mutation authority):
1. `{SIGNAL.affected_area}` — inspect the area identified by the signal evidence.

## Expected Behavior

The signal is resolved through the durable dependency DAG and verified by `FORGE:SIGNAL_RESOLVED` evidence.

## Acceptance Criteria

- [ ] All DAG issues reach their validated terminal outcome.
- [ ] Originating signal is verified resolved and `FORGE:SIGNAL_RESOLVED` is posted.
...
This tracker was created by `/signal-planner`; the dependency DAG is posted as a durable follow-up comment.
TRACKER_EOF
printf '\n<!-- %s -->\n' "$TRACKER_BODY_MARKER" >> "$TRACKER_BODY_TMPFILE"
ISSUE_SKILL_OUTPUT=$(Skill(skill="issue", args="--title \"$TRACKER_TITLE\" --body-file \"$TRACKER_BODY_TMPFILE\" --label signal-planner --label \"priority:{SIGNAL.severity}\""))
...
[ -n "$TRACKER_ISSUE" ] || { echo "ERROR: signal tracker issue hook returned no verified issue number" >&2; exit 1; }
```
...
AGENT_TOKEN="${AGENT_ID:-${HOSTNAME:-signal-planner}-$$}"
mkdir -p "$SCRATCHPAD"
WORK_UNIT_BODY_MARKER="FORGE:BODY-INTEGRITY:signal-work-unit_{work_unit.id}_${AGENT_TOKEN}"
WORK_UNIT_BODY_TMPFILE=$(mktemp "$SCRATCHPAD/signal-work-unit_${AGENT_TOKEN}.XXXXXX.md")
...
Unverified signal-derived hypothesis: {work_unit.root_cause_hypothesis}. Investigation must confirm or reject it.

## Affected Files

Candidate investigation starting points (not mutation authority):
1. `{work_unit.scope}` — inspect the signal-identified area

## Expected Behavior

The validated work unit resolves its portion of the originating signal without regressing the monitored metric or incident condition.
...
ISSUE_EOF
printf '\n<!-- %s -->\n' "$WORK_UNIT_BODY_MARKER" >> "$WORK_UNIT_BODY_TMPFILE"
ISSUE_SKILL_OUTPUT=$(Skill(skill="issue", args="--title \"$WORK_UNIT_TITLE\" --body-file \"$WORK_UNIT_BODY_TMPFILE\" --label \"priority:{work_unit.priority}\" --label \"{work_unit.type}\""))
rm -f "$WORK_UNIT_BODY_TMPFILE"
trap - EXIT
ISSUE_NUM=$(printf '%s\n' "$ISSUE_SKILL_OUTPUT" | sed -n \
  -e 's/.*ISSUE_CREATE_RESULT:CREATED number=\([0-9][0-9]*\).*/\1/p' \
  -e 's/.*ISSUE_CREATE_RESULT:DEDUP number=\([0-9][0-9]*\).*/\1/p' | head -1)
[ -n "$ISSUE_NUM" ] || { echo "ERROR: signal work-unit issue hook returned no verified issue number" >&2; exit 1; }
```

Record each verified `ISSUE_NUM` and update the DAG table. Both `ISSUE_CREATE_RESULT:CREATED` and `ISSUE_CREATE_RESULT:DEDUP` return the authoritative issue number; never fall back to title search.
