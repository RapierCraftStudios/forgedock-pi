...
- For each criterion, verify a bounded closure matrix: producer → every reachable consumer/
  ...
  contract gap requiring a superseding contract and re-plan before remediation. Tests should
  name those omitted boundaries explicitly, not only assert generic caller/dependency wording.
