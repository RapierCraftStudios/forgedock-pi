...
## Prepare a standalone review

A standalone PR or explicit staging review uses the installed helper directly from the target
repository root; it does not create an issue-owner lane or a fabricated issue. Invoke:

`node <package>/specs/helpers/dispatch.mjs standalone-review <plan.json> <new-empty-output-dir>`
...
authorization/report bytes; the joined panel remains incomplete and cannot produce a verdict.
