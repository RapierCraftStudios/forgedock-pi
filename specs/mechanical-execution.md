...
request, and emits `contract` plus `contractDigest` in every lane policy. The generated
owner launch also carries a checked acceptance configuration with exactly the bound
criterion IDs and source hashes; its recipe rejects missing, reordered, generic, or extra
owner report criteria before marking a lane satisfied. Missing, stale, tampered, wrong-issue, or superseded descriptors fail before launch. Child-local config, issue prose,
or a generic `criterion-1` cannot replace the bound input.
