...
function recipe() {
  ...
  requireThat(body, "Packaged dispatcher recipe is missing");
...
function satisfied(result, expected) {
  ...
  requireThat(body.includes(oldSatisfied), "Packaged dispatcher recipe lacks the acceptance boundary");
  body = body.replace(oldSatisfied, exactSatisfied)
    .replace("return launch(key, { ...issue, model: configuredModel }).then((result) => {", "return launch(key, { ...issue, model: configuredModel }).then((rawResult) => {\n    const result = enforceOwnerAcceptance(rawResult, issue.acceptance);")
