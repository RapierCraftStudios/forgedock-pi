...
test("passing container fallback makes missing host imports environment-only", () => {
  const report = classifyVerificationOutput(
    ...
    {
      host: { name: "host Python 3.12 (no project dependencies)", attemptedCommand: "pyright app/main.py" },
      fallbacks: [{ name: "api-test-container", kind: "container", command: "docker compose run --rm api pyright app/main.py", status: "passed" }],
    },
  );
...
test("missing dependency without fallback remains blocked and records skip evidence", () => {
  const report = classifyVerificationOutput("ModuleNotFoundError: No module named 'sqlalchemy'", {
    host: { name: "host Python 3.12", attemptedCommand: "pyright services/api" },
  });
  assert.equal(report.outcome, "blocked");
  assert.equal(report.diagnostics[0]?.kind, "environment-unavailable");
  assert.equal(report.blockingDiagnostics.length, 1);
  assert.match(formatSkippedVerification(report.skipped[0]!), /SKIPPED — environment not provisioned/);
...
test("changed-code errors remain blocking after dependencies resolve", () => {
  const report = classifyVerificationOutput("services/api.py:42: error: Argument of type 'str' is not assignable to parameter of type 'int'", {
    fallbacks: [{ name: "api-venv", kind: "venv", command: ".venv/bin/pyright services/api.py", status: "passed" }],
  });
  assert.equal(report.outcome, "blocked");
...
test("relative module failures are changed-code diagnostics", () => {
  const report = classifyVerificationOutput("src/index.ts:4: error TS2307: Cannot find module './missing'", {
    fallbacks: [{ name: "unrelated-check", kind: "other", command: "npm test", status: "passed" }],
  });
  assert.equal(report.outcome, "blocked");
  assert.equal(report.diagnostics[0]?.kind, "changed-code");
});
...
test("code diagnostics retain blocking precedence on an import line", () => {
  const report = classifyVerificationOutput('Import "fastapi" could not be resolved; TypeError: invalid value', {
...
test("diagnostic report validation rejects contradictory machine results", () => {
  ...
  assert.equal(isVerificationDiagnosticReport({ ...report, outcome: "environment-only" }), false);
});
