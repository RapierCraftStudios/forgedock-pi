...
test("lane promotion events persist queue, lease, and merge receipt state", () => {
  ...
  assert.throws(
    () => applyOrchestrationEvent(state, next(state, "orchestration.cancelled", { reason: "late cancellation" }, "cancel-after-promotion")),
    (error) => error instanceof Error && "code" in error && error.code === "promotion-in-progress",
  );
