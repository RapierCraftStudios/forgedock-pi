...
test("RPC work-on launch binds the nested-review runtime contract", async () => {
  ...
  assert.match(
    spawn.params.task,
    /one synchronous workflowScript per complete panel attempt/,
  );
  ...
  assert.match(spawn.params.task, /investigation-first.*FORGE:CONTRACT.*Builder Brief/s);
  assert.match(spawn.params.task, /live path.*callers\/consumers.*historical constraints/s);
  assert.match(spawn.params.task, /patch-defect.*contract-gap/s);
  assert.doesNotMatch(spawn.params.task, /pre-build challenger|adaptive pre-build challenge|timeoutMs=300000/i);
...
test("materialized project agents preserve nested work-on hierarchy for async runners", async () => {
  ...
  try {
    ...
    assert.match(workOn, /forge_checkpoint/);
    assert.match(workOn, /forge_finalize_work_on/);
    assert.doesNotMatch(workOn, /forge_finalize_node/);
    ...
    assert.match(domainReviewer, /^completionGuard: false$/m);
    assert.match(refresh, /tools: .*subagent\b/);
    assert.doesNotMatch(refresh, /tools: .*forge_run_review_panel/);
...
test("runtime Forge hierarchy keeps full work-on and reviewer boundaries", () => {
  assert.equal(
  ...
  assert.equal(
    (FORGE_REFRESH_REVIEW_TOOLS as readonly string[]).includes(
      "forge_run_review_panel",
    ),
    false,
  );
