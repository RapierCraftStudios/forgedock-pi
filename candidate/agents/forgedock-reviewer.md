...
sections requested by the task. Call `forge_publish_reviewer` exactly once after analysis. If
publication fails, keep the saved body/report evidence and report the failure; never rerun the
review. Return one compact `FORGE_REVIEW_RESULT` line only after the publication attempt.
